...
/**
 * GET /api/live-list
 * 현재 YouTube에서 방송 중인 리스트를 가져옵니다.
 * 
 * Query parameters:
 * - platform: "youtube" (선택사항, 없으면 전체)
 */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const platform = searchParams.get("platform") as "youtube" | null;
    ...
    console.log(`[API] 📊 Platform breakdown: YouTube=${youtubeCount}, Total=${result.liveList.length}`);
        ...
        debug: {
          ...
          message: result.liveList.length === 0 
            ? (result as any).diagnosticInfo?.youtubeQuotaExceeded
              ...
              : "서버 터미널에서 [LiveList], [YouTube]로 시작하는 로그를 확인하세요. 특히 YouTube API 할당량 초과 메시지를 확인하세요."
